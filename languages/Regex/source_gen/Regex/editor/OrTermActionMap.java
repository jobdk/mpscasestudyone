  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (DeletionApproverUtil.approve(editorContext, node, "rightOrTermCellId")) {
          return;
        }
        SNode leftTerm = SNodeOperations.copyNode(SLinkOperations.getTarget(node, LINKS.leftTerm$12dD));
        SNodeOperations.replaceWithAnother(node, leftTerm);
        SelectionUtil.selectLabelCellAnSetCaret(editorContext, leftTerm, SelectionManager.LAST_CELL, -1);
      }
